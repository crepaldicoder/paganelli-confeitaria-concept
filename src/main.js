...
// A intro escreve o logo oficial: os paths de tinta de public/images/logo-paganelli.svg
// entram aqui literalmente e são apenas revelados por máscara — nada é redesenhado.
import officialLogoFile from '../public/images/logo-paganelli.svg?raw';
...
const officialLogo=(()=>{
 const viewBox=(officialLogoFile.match(/viewBox="([^"]+)"/)||[])[1]||'0 0 1876 462';
 const ink=officialLogoFile.slice(officialLogoFile.indexOf('<g class="ink">'));
 const paths=[...ink.matchAll(/<path\b[^>]*\bd="([^"]+)"/g)].map(m=>m[1]);
 const fill=(officialLogoFile.match(/\.ink\s+path\s*\{[^}]*?fill:\s*(#[0-9a-fA-F]{3,8})/)||[])[1]||'#f4b7c8';
 const frame=viewBox.trim().split(/[\s,]+/).map(Number);
 return {viewBox,paths,fill,width:frame[2]||1876,height:frame[3]||462};
})();
const WRITE={feather:96,slant:11};
const introLogo=()=>{
 const strokes=officialLogo.paths.map(d=>`<path d="${d}"/>`).join('');
 const brush=`<g transform="skewX(${-WRITE.slant})"><rect class="pg-sweep" x="-2600" y="-320" width="2600" height="1120"`;
 return `<svg class="intro-logo" viewBox="${officialLogo.viewBox}" aria-hidden="true" focusable="false"><defs>`
 +`<linearGradient id="pg-ink-edge" gradientUnits="userSpaceOnUse" x1="${-WRITE.feather}" y1="0" x2="0" y2="0"><stop offset="0" stop-color="#fff"/><stop offset=".42" stop-color="#fff" stop-opacity=".93"/><stop offset=".76" stop-color="#fff" stop-opacity=".46"/><stop offset="1" stop-color="#fff" stop-opacity="0"/></linearGradient>`
 +`<linearGradient id="pg-wet-edge" gradientUnits="userSpaceOnUse" x1="-430" y1="0" x2="0" y2="0"><stop offset="0" stop-color="#fff" stop-opacity="0"/><stop offset=".52" stop-color="#fff" stop-opacity=".13"/><stop offset=".84" stop-color="#fff" stop-opacity=".44"/><stop offset=".96" stop-color="#fff" stop-opacity=".2"/><stop offset="1" stop-color="#fff" stop-opacity="0"/></linearGradient>`
 +`<mask id="pg-hand-mask" maskUnits="userSpaceOnUse" x="-800" y="-420" width="3600" height="1320">${brush} fill="url(#pg-ink-edge)"/></g></mask>`
 +`<mask id="pg-wet-mask" maskUnits="userSpaceOnUse" x="-800" y="-420" width="3600" height="1320">${brush} fill="url(#pg-wet-edge)"/></g></mask>`
 +`</defs><g class="pg-hand" fill="${officialLogo.fill}" mask="url(#pg-hand-mask)">${strokes}</g>`
 +`<g class="pg-wet" mask="url(#pg-wet-mask)">${strokes}</g>`
 +`<g class="pg-tittle" fill="${officialLogo.fill}"></g></svg>`;
};

document.querySelector('#app').innerHTML=`
<a class="skip" href="#conteudo">Pular para o conteúdo</a>
<div class="build-intro" aria-hidden="true"><div class="intro-panel intro-panel--left"></div><div class="intro-panel intro-panel--right"></div><div class="intro-stage"><span class="intro-index">SÃO JOSÉ DO RIO PRETO · 1947</span><div class="intro-writing">${introLogo()}</div><p>uma receita construída<br>traço por traço</p></div></div><div class="scroll-progress" aria-hidden="true"><i></i></div>
<header class="topbar">
...
// Escrita da intro: uma máscara inclinada e esfumada corre da esquerda para a direita sobre os
// paths oficiais. O ritmo é medido nesses mesmos paths — a caneta demora onde o traço é denso e
// corre nas ligações —, então o que aparece é sempre o preenchimento do logo, nunca um desenho novo.
const startIntroWriting=(motion)=>{
 const logo=document.querySelector('.intro-logo');if(!logo)return;
 const hand=logo.querySelector('.pg-hand'),wet=logo.querySelector('.pg-wet'),tittle=logo.querySelector('.pg-tittle');
 const settle=()=>{hand.removeAttribute('mask');wet.remove();tittle.style.opacity='1'};
 if(!motion||typeof logo.animate!=='function'){settle();return}
 const slant=Math.tan(WRITE.slant*Math.PI/180);
 const copies=[...wet.children];
 // O pingo do i é curto demais para nascer no meio da varredura: sai do grupo varrido e cai no fim.
 try{[...hand.children].forEach((path,i)=>{const b=path.getBBox();if(b.width<officialLogo.width*.045&&b.height<officialLogo.height*.14){tittle.append(path);copies[i].remove()}})}catch(e){}
 const swept=[...hand.children],reveal=[];
 try{
  const lengths=swept.map(path=>path.getTotalLength()),total=lengths.reduce((a,b)=>a+b,0)||1;
  swept.forEach((path,i)=>{const steps=Math.max(2,Math.round(900*lengths[i]/total));for(let s=0;s<=steps;s++){const p=path.getPointAtLength(lengths[i]*s/steps);reveal.push(p.x+slant*p.y)}});
 }catch(e){}
 if(reveal.length<8){try{const b=logo.getBBox();reveal.push(b.x,b.x+b.width+slant*(b.y+b.height))}catch(e){settle();return}}
 const from=Math.min(...reveal)-22,last=Math.max(...reveal);
 const span=last+WRITE.feather*.55-from,cols=56,density=new Array(cols).fill(0);
 reveal.forEach(u=>{const c=Math.floor((u-from)/span*cols);if(c>=0&&c<cols)density[c]++});
 const flow=density.map((v,i)=>(density[i-1]||0)*.22+v*.56+(density[i+1]||0)*.22);
 const pace=flow.map(v=>Math.pow(v+Math.max(...flow,1)*.12,.74)),beat=pace.reduce((a,b)=>a+b,0);
 const frames=[{transform:`translateX(${from.toFixed(1)}px)`,offset:0}];
 let acc=0;
 pace.forEach((v,i)=>{acc+=v;frames.push({transform:`translateX(${(from+span*(i+1)/cols).toFixed(1)}px)`,offset:+(acc/beat*.88).toFixed(4)})});
 frames[frames.length-1].easing='cubic-bezier(.22,.66,.3,1)';
 frames.push({transform:`translateX(${(last+WRITE.feather+26).toFixed(1)}px)`,offset:1});
 const compact=matchMedia('(max-width:700px)').matches;
 const delay=compact?300:360,duration=compact?2000:2300;
 logo.querySelectorAll('.pg-sweep').forEach(rect=>rect.animate(frames,{duration,delay,fill:'both'}));
 if(tittle.children.length)tittle.animate([{opacity:0,transform:'scale(.78)'},{opacity:1,transform:'scale(1)'}],{duration:compact?230:270,delay:delay+duration+(compact?40:70),fill:'both',easing:'cubic-bezier(.2,.86,.3,1)'});
 setTimeout(settle,delay+duration+(compact?330:400));
};

const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
const revealTargets=[...document.querySelectorAll('.reveal,.construct,.image-build')];
startIntroWriting(!reduced);
if(reduced){document.body.classList.add('site-ready');revealTargets.forEach(el=>el.classList.add(el.classList.contains('reveal')?'visible':'built'))}
 ...
 const updateMotion=()=>{const max=document.documentElement.scrollHeight-innerHeight;progress?.style.setProperty('--progress',Math.max(0,Math.min(1,scrollY/(max||1))));document.querySelectorAll('.image-build img').forEach(img=>{const frame=img.closest('.image-build'),r=frame.getBoundingClientRect(),d=(r.top+r.height/2-innerHeight/2)/innerHeight;frame.style.setProperty('--parallax-y',`${Math.max(-22,Math.min(22,-d*18))}px`)});ticking=false};
