...
if(reduced){document.body.classList.add('site-ready');revealTargets.forEach(el=>el.classList.add(el.classList.contains('reveal')?'visible':'built'))}
 ...
 const progress=document.querySelector('.scroll-progress'),progressLabel=progress?.querySelector('.recipe-progress__label'),progressDots=[...(progress?.querySelectorAll('.recipe-progress__dots b')||[])],chapterStops=[['inicio','Início'],['oficio','O ofício'],['mesa','Da vitrine'],['avaliacoes','Avaliações'],['visita','Visite']].map(([id,label])=>({el:document.getElementById(id),label})),scenePanels=[...document.querySelectorAll('.quote-band,.heritage-number,.before-you-go')];let ticking=false,lastChapter=-1;
 const updateMotion=()=>{const max=document.documentElement.scrollHeight-innerHeight,p=Math.max(0,Math.min(1,scrollY/(max||1)));progress?.style.setProperty('--progress',p);document.documentElement.style.setProperty('--paper-x',`${Math.sin(scrollY/620)*6}px`);document.documentElement.style.setProperty('--paper-y',`${Math.sin(scrollY/430)*4}px`);let active=0;chapterStops.forEach((stop,i)=>{if(stop.el&&stop.el.getBoundingClientRect().top<=innerHeight*.45)active=i});if(progressLabel&&active!==lastChapter){progressLabel.textContent=chapterStops[active]?.label||'Início';progressLabel.animate([{opacity:.2,transform:'rotate(180deg) translateY(7px)'},{opacity:1,transform:'rotate(180deg) translateY(0)'}],{duration:360,easing:'cubic-bezier(.16,1,.3,1)'});lastChapter=active}progressDots.forEach((dot,i)=>dot.classList.toggle('active',i<=active));scenePanels.forEach(panel=>{const r=panel.getBoundingClientRect(),local=Math.max(-1,Math.min(1,(r.top+r.height/2-innerHeight/2)/innerHeight));panel.style.setProperty('--scene-shift-x',`${local*10}px`);panel.style.setProperty('--scene-shift-y',`${local*-4}px`)});document.querySelectorAll('.image-build img').forEach((img,index)=>{const frame=img.closest('.image-build');if(innerWidth<=900){frame.style.setProperty('--parallax-y','0px');frame.style.setProperty('--parallax-x','0px');frame.style.setProperty('--parallax-scale','1.045');return}const r=frame.getBoundingClientRect(),d=(r.top+r.height/2-innerHeight/2)/innerHeight,depth=.72+(index%3)*.14;frame.style.setProperty('--parallax-y',`${Math.max(-22,Math.min(22,-d*17*depth))}px`);frame.style.setProperty('--parallax-x',`${Math.max(-5,Math.min(5,d*(index%2?3:-3)))}px`);frame.style.setProperty('--parallax-scale',String(1.035+Math.min(.018,Math.abs(d)*.012)))});ticking=false};
